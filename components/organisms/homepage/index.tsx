...
import styles from '/styles/components/organisms/homepage.module.scss'
...
import { CarofTheMonth } from 'components/molecules'
...
const Homepage = () => {
  ...
  const getCarOfTheMonth = async () => {
    try {
      ...
      setCarOfTheMonthData(carofmonth.data.data)
    } catch (e: any) {
      throw new Error(e)
    }
  }
  ...
  //   try {
  //     const recommendation: any = await api.getRecommendation()
  //     saveRecommendation(recommendation.data.carRecommendations)
  //   } catch {
  ...
  return (
    <>
      ...
      {!isLeadsFormSectionVisible && <CSAButton onClick={scrollToLeadsForm} />}
    </>
  )
}
