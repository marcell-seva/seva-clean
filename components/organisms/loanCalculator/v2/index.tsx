...
export function LoanCalculatorPageV2() {
  ...
  const getAutofilledDownPayment = () => {
    ...
    } else if (kkForm?.downPaymentAmount) {
  ...
  const [forms, setForms] = useState<FormLCState>({
    ...
    model: {
      brandName: brand
        ...
        : kkForm?.model.brandName || '',
      modelName: kkForm?.model.modelName || '',
      modelId: kkForm?.model.modelId || '',
      modelImage:
        kkForm?.model.modelImage || (CarSillhouete as unknown as string),
      loanRank: kkForm?.model.loanRank,
    },
    variant: {
      variantId: '',
      variantName: '',
  ...
  const initialFillCarVariant = () => {
    if (carVariantList.length > 0) {
      const variantData = variant
        ? carVariantList.filter(
            (item) =>
              item.name
                .toLowerCase()
                .replaceAll(/ +/g, '-')
  ...
  const autofillCarVariantData = () => {
    const currentVariantId =
      ...
      (kkForm?.model.modelId === forms.model?.modelId &&
  ...
  const handleChange = (name: string, value: any) => {
    ...
    if (name === 'model') {
      if (value.modelId !== forms.model?.modelId)
        setForms({
          ...forms,
          [name]: value,
          variant: variantEmptyValue,
            ...
            <div id="loan-calculator-form-dp">
              <DpForm
                ...
                isAutofillValueFromCreditQualificationData={
                  (!!storedFilter || !!kkForm) && !isUserChooseVariantDropdown
                }
